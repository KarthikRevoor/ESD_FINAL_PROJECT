...
void SPI_SD_Init() {
    SD_Deselect(); // Ensure CS is high during initialization
}

void init_system() {
	ILI9341_Init();               // Initialize the LCD
    SPI_SD_Init();            // Initialize SD card SPI
    if (f_mount(&USERFatFS, USERPath, 1) == FR_OK) {
        ILI9341_FillScreen(0x0000); // Clear screen
        DrawString(10, 10, "SD Card Mounted", 0x07E0, 0x0000, 1); // Green Text
    } else {
        DrawString(10, 10, "SD Mount Failed!", 0xF800, 0x0000, 1); // Red Text
        while (1); // Halt
    }
}
...
uint8_t SD_SendCommand(uint8_t cmd, uint32_t arg) {
    ...
    uint8_t commandFrame[6] = {
        0x40 | cmd,                 // Command byte
        (arg >> 24) & 0xFF,         // Argument MSB
        (arg >> 16) & 0xFF,
        (arg >> 8) & 0xFF,
        arg & 0xFF,                 // Argument LSB
        (cmd == 0 ? 0x95 : 0x87)    // CRC for CMD0 and CMD8, dummy otherwise
    };
    HAL_SPI_Transmit(&hspi2, commandFrame, 6, HAL_MAX_DELAY);

    // Wait for response (R1 format, 1 byte)
    for (int i = 0; i < 8; i++) {
        HAL_SPI_Receive(&hspi2, &response, 1, HAL_MAX_DELAY);
        if ((response & 0x80) == 0) break;
    }

    // Deselect the SD card after the command
    SD_Deselect();
